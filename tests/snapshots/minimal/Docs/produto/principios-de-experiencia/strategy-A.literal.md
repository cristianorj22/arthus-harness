...
> **Strategy A (literal default).** Este arquivo chega pré-preenchido com as 4 sensações + 5 réguas operacionais que emergiram em GoParty. **Use, edite, ou troque.** Quando algo soar específico de GoParty (escrow, anfitrião, reserva), substitua pelo equivalente do seu domínio. Os comentários `<!-- example: GoParty — replace -->` marcam onde.
>
...
<!-- example: GoParty preenche [X] = "pagar e poder dormir tranquilo".
...
<!-- example: GoParty: "meu dinheiro está protegido". Substitua pela frase âncora
...
Sentida no momento mais sensível da jornada (em GoParty: checkout, primeira reserva, ao confirmar pagamento). Construída por **linguagem clara sobre o que está acontecendo, prazos visíveis, status acompanhável**. Quebrada por jargão técnico, status enigmáticos ("processando…"), ausência de quem-é-responsável-pelo-quê.

### 2. Alívio — "[achei o lugar, posso descansar]"

<!-- example: GoParty: "achei o espaço, evento dos sonhos parou de ser incerteza".
...
<!-- example: GoParty: pós-evento, liberação de pagamento. SaaS: deploy concluído com sucesso.
...
> **Source histórico**: este documento foi extraído literal de `go-party-venue-hub/Docs/produto/principios-de-experiencia.md` no bootstrap via `arthus-harness v1.0.0`. Edição é encorajada.
