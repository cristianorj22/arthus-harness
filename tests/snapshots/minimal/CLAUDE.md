...
| `/code-review` | Antes de commit local (escopo pequeno) ou antes de merge em main. Roda 3 reviewers em paralelo. |
...
|---|---|---|
| `code-reviewer` | Toda mudança de código. Carrega skill `experience-principles`. Inclui SPEC drift check (MEDIUM em mudança de superfície pública sem update). | core |
...
| `security-reviewer` | Auth, secrets, edge functions, validação de input externo, OWASP Top 10.  | core |
...
3 skills core do harness (mais adicionadas pelos plugins escolhidos: none):
...
- UI nova → ler **`Docs/produto/principios-de-experiencia.md`** (sensação a preservar). 
