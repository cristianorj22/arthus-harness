...
| Pagamentos | Asaas (BR — PIX/Boleto/Cartão) |
...
| `ASAAS_API_URL` `ASAAS_API_KEY` | Edge functions. |
| `ASAAS_WEBHOOK_TOKEN` | Edge functions. Validação HMAC do webhook. |
| `<TODO>` | <TODO> |
...
| `npm run i18n:check` | Paridade de locales — skipa se `src/locales/` não existe |
| `npm run db:reset` | Reseta DB local (Supabase CLI) |
| `npm run check:all` | `lint && type-check && design:check && i18n:check` |
...
|---|---|---|
| `code-reviewer` | Toda mudança de código. Carrega skill `experience-principles` + `i18n-source-of-truth` + `design-system-enforcement`. Inclui SPEC drift check (MEDIUM em mudança de superfície pública sem update). | core |
...
| `security-reviewer` | Auth, secrets, edge functions, validação de input externo, OWASP Top 10. Carrega skill `supabase-rls-pattern`. Carrega skill `asaas-integration`.  | core |
...
3 skills core do harness (mais adicionadas pelos plugins escolhidos: supabase, design-system-pipeline, payment-asaas, i18n, e2e-playwright, journey-mapping):
...
| `i18n-source-of-truth` | Source-of-truth pattern, ICU plurals, paridade entre locales. |
| `supabase-rls-pattern` | RLS por tabela, `(select auth.uid())`, índices obrigatórios em FK referenciadas por policy. |
| `asaas-integration` | Webhook HMAC, idempotência, error pt-BR, log redaction. |
